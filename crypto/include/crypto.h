...
typedef struct hw_crypto_sym_req {
    ...
    uint32_t iv_len;
    const uint8_t *aad;
    uint32_t aad_len; /* 仅 AEAD/GCM 使用 */
    const uint8_t *in;
    uint32_t in_len; /* encrypt: 明文; decrypt: 密文 */
    uint8_t *out;
    uint32_t out_cap; /* encrypt: >= in_len + 16; decrypt: >= in_len */
    uint8_t *tag;
    uint32_t tag_cap; /* GCM 标签缓冲, >= 16; 非 GCM 可传 NULL */
} hw_crypto_sym_req_t;
...
typedef struct hw_crypto_ops {
    ...
    /* --- 密钥管理 --- */
    int (*gen_sym_key)(const hw_crypto_keygen_req_t *req, hw_crypto_symkey_resp_t *resp);
    int (*gen_rsa_key)(const hw_crypto_keygen_req_t *req, hw_crypto_rsa_resp_t *resp);
    /* PEM 导入导出：导入校验并返回长度；导出为字符串拷贝 */
    int (*import_pem)(const char *pem, size_t len, int expected_asym, int *is_asym);
    int (*export_pem)(const char *in_pem, size_t in_len, int want_priv, char *out_pem,
                      size_t *out_len);
