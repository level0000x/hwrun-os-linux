...
static int cipher_lookup(int cipher, cipher_spec *out) {
    ...
    case HWRUN_CRYPTO_CIPHER_AES_128_CBC:
        out->cipher = EVP_aes_128_cbc(); out->key_len = 16; out->iv_len = 16; out->is_aead = 0; return 0;
    case HWRUN_CRYPTO_CIPHER_AES_256_CBC:
        out->cipher = EVP_aes_256_cbc(); out->key_len = 32; out->iv_len = 16; out->is_aead = 0; return 0;
    case HWRUN_CRYPTO_CIPHER_AES_128_GCM:
        out->cipher = EVP_aes_128_gcm(); out->key_len = 16; out->iv_len = 12; out->is_aead = 1; return 0;
    case HWRUN_CRYPTO_CIPHER_AES_256_GCM:
        out->cipher = EVP_aes_256_gcm(); out->key_len = 32; out->iv_len = 12; out->is_aead = 1; return 0;
    default:
        return -ENOTSUP;
    }
}
...
static const EVP_MD *hash_md(int algo) {
    switch (algo) {
    case HWRUN_CRYPTO_HASH_SHA256: return EVP_sha256();
    case HWRUN_CRYPTO_HASH_SHA512: return EVP_sha512();
    case HWRUN_CRYPTO_HASH_MD5:    return EVP_md5();
    default:                       return NULL;
    }
}
 ...
 * 哈希
 * ============================================================ */
static int crypto_hash_impl(const hw_crypto_hash_req_t *req,
 ...
 * HMAC
 * ============================================================ */
static int crypto_hmac_impl(const hw_crypto_hmac_req_t *req,
    ...
    if (HMAC(md, req->key, (int)req->key_len,
             req->data, req->data_len, req->out, &olen) == NULL)
        return -EIO;
    resp->out_len = (uint32_t)olen;
    return HWRUN_OK;
}
 ...
 * 对称加密 / 解密
 * ============================================================ */
static int crypto_encrypt_impl(const hw_crypto_sym_req_t *req,
                               hw_crypto_sym_resp_t *resp) {
    cipher_spec spec;
    EVP_CIPHER_CTX *ctx = NULL;
    int olen = 0, tlen = 0, ret = -EINVAL;

    if (!req || !resp || !req->key || !req->in || !req->out)
        return -EINVAL;
    if (cipher_lookup(req->cipher, &spec) != 0)
        return -ENOTSUP;
    if ((int)req->key_len != spec.key_len)
        return -EINVAL;
    if ((int)req->iv_len != spec.iv_len)
        return -EINVAL;
    ...
    if (spec.is_aead) {
        ...
        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)req->iv_len, NULL) != 1) goto out;
        ...
        /* 密文长度 == 明文长度 */
        if (req->out_cap < (uint32_t)req->in_len) { ret = -ENOMEM; goto out; }
        ...
        if (req->tag && req->tag_cap >= HW_CRYPTO_GCM_TAG_LEN) {
            if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, HW_CRYPTO_GCM_TAG_LEN, req->tag) != 1) goto out;
        ...
        if (req->out_cap < (uint32_t)(req->in_len + 16)) { ret = -ENOMEM; goto out; }
        if (EVP_EncryptUpdate(ctx, req->out, &olen, req->in, (int)req->in_len) != 1) goto out;
        if (EVP_EncryptFinal_ex(ctx, req->out + olen, &tlen) != 1) goto out;
        olen += tlen;
        resp->tag_len = 0;
...
static int crypto_decrypt_impl(const hw_crypto_sym_req_t *req,
                               hw_crypto_sym_resp_t *resp) {
    cipher_spec spec;
    EVP_CIPHER_CTX *ctx = NULL;
    int olen = 0, tlen = 0, ret = -EINVAL;
    ...
    if (spec.is_aead) {
        if (!req->tag || req->tag_cap < HW_CRYPTO_GCM_TAG_LEN) { ret = -EINVAL; goto out; }
        ...
        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, HW_CRYPTO_GCM_TAG_LEN,
                                (void *)(uintptr_t)req->tag) != 1) goto out;
        if (req->aad && req->aad_len) {
...
static int crypto_rsa_bits_from_algo(int algo) {
    switch (algo) {
    case HWRUN_CRYPTO_ASYM_RSA_2048: return 2048;
    case HWRUN_CRYPTO_ASYM_RSA_4096: return 4096;
    default:                         return 0;
    }
}
 ...
 * RSA 密钥对生成（PEM 导出）
 * ============================================================ */
static int crypto_gen_rsa_key_impl(const hw_crypto_keygen_req_t *req,
    ...
    if (!pbio) goto out;
    if (PEM_write_bio_PKCS8PrivateKey(pbio, pkey, NULL, NULL, 0, NULL, NULL) != 1)
        goto out;
 ...
 * PEM 导入 / 导出
 * ============================================================ */
static int crypto_import_pem_impl(const char *pem, size_t len,
                                  int expected_asym, int *is_asym) {
    ...
    if (expected_asym) {
        ...
        if (strstr(p, "PRIVATE KEY") || strstr(p, "PUBLIC KEY") ||
            strstr(p, "RSA PRIVATE KEY") || strstr(p, "RSA PUBLIC KEY")) {
...
static int crypto_export_pem_impl(const char *in_pem, size_t in_len,
                                  int want_priv, char *out_pem, size_t *out_len) {
    size_t n;
 ...
 * RSA 签名 / 验签
 * ============================================================ */
static int crypto_sign_impl(const hw_crypto_sign_req_t *req,
                            hw_crypto_sign_resp_t *resp) {
    BIO *bio = NULL;
    EVP_PKEY *pkey = NULL;
    ...
    if (!req || !resp || !req->priv_pem || !req->data || !req->sig)
        return -EINVAL;
    ...
    if (!pkey) { ret = -EILSEQ; goto out; }

    mctx = EVP_MD_CTX_new();
    if (!mctx) { ret = -ENOMEM; goto out; }
    ...
    if ((uint32_t)siglen > req->sig_cap) { ret = -ENOMEM; goto out; }
...
static int crypto_verify_impl(const hw_crypto_verify_req_t *req,
                              hw_crypto_verify_resp_t *resp) {
    BIO *bio = NULL;
    EVP_PKEY *pkey = NULL;
    EVP_MD_CTX *mctx = NULL;
    const EVP_MD *md;
    ...
    if (!req || !resp || !req->pub_pem || !req->data || !req->sig)
        return -EINVAL;
...
static int crypto_capabilities_impl(hw_crypto_capability_t *cap) {
    ...
    snprintf(cap->backend, sizeof(cap->backend), "libcrypto %s",
