...
typedef struct hw_np_ops {
    ...
    /* UDP 发送到指定目标 */
    ssize_t (*sendto)(int fd, const void *data, size_t len, int flags, const struct sockaddr *dest,
                      socklen_t addrlen);
    /* UDP 接收并回填发送方地址 */
    ...
    int (*resolve)(const char *host, unsigned port, struct sockaddr_storage *out, int max,
                   int *out_count);
