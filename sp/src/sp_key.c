 ...
 * 算法 -> genpkey 参数
 * ============================================================ */
static int sp_key_gen_cmd(int algo, char *cmd, size_t cap, const char *priv, const char *pub) {
 ...
 * 存储 PEM 到密钥条目（调用方须持锁）
 * ============================================================ */
static sp_key_entry_t *sp_key_new_entry(int algo, const char *desc, const char *priv,
                                        const char *pub) {
...
int sp_gen_key_pair(int algo, const char *desc, uint32_t *key_id) {
    ...
    if (algo != SP_ASYM_RSA_2048 && algo != SP_ASYM_RSA_4096 && algo != SP_ASYM_ECDSA_P256)
        return SP_ENOTSUP;
...
int sp_get_public_key(uint32_t key_id, char *pem_out, uint32_t pem_cap) {
    ...
    if (rc == SP_OK) {
        if (strlen(e->pub_pem) + 1 > pem_cap) {
            rc = SP_ENOMEM;
        } else {
            memcpy(pem_out, e->pub_pem, strlen(e->pub_pem) + 1);
        }
    }
    pthread_mutex_unlock(&g_sp_ctx.lock);
    return rc;
...
int sp_export_private_key(uint32_t key_id, char *pem_out, uint32_t pem_cap) {
    ...
    if (rc == SP_OK) {
        if (strlen(e->priv_pem) + 1 > pem_cap) {
            rc = SP_ENOMEM;
        } else {
            memcpy(pem_out, e->priv_pem, strlen(e->priv_pem) + 1);
        }
    }
    pthread_mutex_unlock(&g_sp_ctx.lock);
    return rc;
 ...
 * ============================================================ */
int sp_import_key_pair(int algo, const char *desc, const char *priv_pem, uint32_t *key_id) {
    ...
    char cmd[16384];
    snprintf(cmd, sizeof(cmd), "openssl pkey -in %s -pubout -out %s", sp_q(priv, qi, sizeof(qi)),
             sp_q(pub, qo, sizeof(qo)));
...
int sp_list_keys(char *buf, uint32_t buf_cap) {
    ...
    for (sp_key_entry_t *e = g_sp_ctx.keys; e; e = e->next) {
        int n = snprintf(buf + used, buf_cap - used, "%u\t%s\t%s\t%s\n", e->key_id,
                         e->algo == SP_ASYM_RSA_2048
                             ? "RSA-2048"
                             : (e->algo == SP_ASYM_RSA_4096 ? "RSA-4096" : "ECDSA-P256"),
                         e->revoked ? "REVOKED" : "ACTIVE", e->desc);
        if (n < 0 || (size_t)n >= buf_cap - used) {
            pthread_mutex_unlock(&g_sp_ctx.lock);
            return SP_ENOMEM;
        }
        used += (size_t)n;
...
int sp_sign(uint32_t key_id, const uint8_t *data, uint32_t len, char *sig_b64_out,
    ...
    if (!inpath || !keypath || !sigpath) {
        free(privPem);
        return SP_EIO;
    }
    if (sp_write_all(inpath, data, len) != SP_OK) {
        free(privPem);
        return SP_EIO;
    }
    if (sp_write_all(keypath, privPem, strlen(privPem)) != SP_OK) {
        free(privPem);
        return SP_EIO;
    }
    free(privPem);
...
int sp_verify(uint32_t key_id, const uint8_t *data, uint32_t len, const char *sig_b64, int *valid) {
    ...
    *valid = 0;
    ...
    if (!inpath || !keypath || !sigpath || !outpath || !errpath) {
        free(pubPem);
        return SP_EIO;
    }
    if (sp_write_all(inpath, data, len) != SP_OK) {
    ...
    if (sp_write_all(keypath, pubPem, strlen(pubPem)) != SP_OK) {
        free(pubPem);
        return SP_EIO;
    }
    free(pubPem);
 ...
 * 回调装配
 * ============================================================ */
static int ops_list_keys_impl(char *b, uint32_t c) {
    return sp_list_keys(b, c);
}
