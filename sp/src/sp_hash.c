...
static const char *sp_hash_flag(int algo) {
    switch (algo) {
        case SP_HASH_SHA1:     return "sha1";
        case SP_HASH_SHA256:   return "sha256";
        case SP_HASH_SHA512:   return "sha512";
        case SP_HASH_SHA3_256: return "sha3-256";
        case SP_HASH_SHA3_512: return "sha3-512";
        default:               return NULL;
    }
}
...
static int sp_hash_outlen(int algo) {
    switch (algo) {
        case SP_HASH_SHA1:     return 20;
        case SP_HASH_SHA256:   return 32;
        case SP_HASH_SHA512:   return 64;
        case SP_HASH_SHA3_256: return 32;
        case SP_HASH_SHA3_512: return 64;
        default:               return 0;
    }
}
 ...
 * 在本环境的不可靠行为）。
 * ============================================================ */
static int sp_hash_bin_file(int algo, const char *path,
...
int sp_hash_compute(int algo, const uint8_t *data, uint32_t len,
    ...
    int rc = sp_hash_bin_file(algo, inpath, out, out_len ? *out_len : SP_MAX_HASH_LEN,
                              out_len);
    unlink(inpath);
    return rc;
}
...
/* 返回十六进制摘要字符串（内存数据） */
static int ops_hash_hex(int algo, const uint8_t *data, uint32_t len,
...
/* 返回二进制摘要（内存数据） */
static int ops_hash(int algo, const uint8_t *data, uint32_t len,
                    uint8_t *out, uint32_t *out_len) {
    return sp_hash_compute(algo, data, len, out, out_len);
}

/* 文件哈希（二进制） */
static int ops_hash_file(int algo, const char *path,
                         uint8_t *out, uint32_t *out_len) {
    return sp_hash_bin_file(algo, path, out, *out_len, out_len);
}

/* 文件哈希（十六进制） */
static int ops_hash_file_hex(int algo, const char *path,
                             char *out_hex, uint32_t hex_cap) {
    uint8_t digest[SP_MAX_HASH_LEN];
    uint32_t dlen = sizeof(digest);
...
/* 校验内存数据哈希是否匹配预期 */
static int ops_verify_hash(int algo, const uint8_t *data, uint32_t len,
...
/* 校验文件哈希是否匹配预期的十六进制串 */
static int ops_verify_file_hash(int algo, const char *path,
    ...
    size_t elen = 0;
    if (sp_from_hex(expected_hex, expect, sizeof(expect), &elen) != SP_OK)
        return SP_EINVAL;
