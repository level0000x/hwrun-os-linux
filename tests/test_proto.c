...
static hw_plugin_t *load_plugin(hw_bus_t *bus, const char *so_path) {
    ...
    for (int i = 0; i < self->provides_count; i++) {
        ...
        hw_metaproto_register(&bus->meta, self->provides[i], HWRUN_PROTOCOL_VERSION, self->id,
                              impl ? impl : (void *)self);
    }
    ...
    /* 挂到总线插件链表头部，便于统一清理 */
    self->next = bus->plugins;
    if (bus->plugins) bus->plugins->prev = self;
    bus->plugins = self;
    return self;
}
...
static hw_protocol_route_t *resolve(hw_bus_t *bus, const char *proto) {
    ...
    if (hw_metaproto_resolve(&bus->meta, proto, NULL, &r) != HWRUN_OK || !r) return NULL;
    return r;
}
...
int main(void) {
    ...
    if (check_impl(&bus, "HAP") == HWRUN_OK) {
        ...
        CHECK(hap->get_cpu_info && hap->get_cpu_info(&cpu) == HWRUN_OK,
              "HAP.get_cpu_info 调用成功");
    ...
    if (check_impl(&bus, "FSP") == HWRUN_OK) {
        ...
        CHECK(fsp->statfs && fsp->statfs("/tmp", &fs) == HWRUN_OK, "FSP.statfs(/tmp) 调用成功");
        CHECK(fsp->access && fsp->access("/tmp", 0) == HWRUN_OK, "FSP.access(/tmp, F_OK) 存在");
