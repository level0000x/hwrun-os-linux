...
int git_ops_checkout(const char *ref) {
    ...
    int ret = r->success ? 0 : -1;
    if (!r->success)
        HWAPI_LOGE(PLUGIN_ID, "git checkout 失败: %s", r->stdout_buf ? r->stdout_buf : "");
    git_result_free(r);
    return ret;
}
...
int git_ops_reset(const char *ref, int hard) {
    ...
    int ret = r->success ? 0 : -1;
    if (!r->success)
        HWAPI_LOGE(PLUGIN_ID, "git reset 失败: %s", r->stdout_buf ? r->stdout_buf : "");
    git_result_free(r);
    return ret;
}
...
int git_ops_revert(const char *commit) {
    ...
    int ret = r->success ? 0 : -1;
    if (!r->success)
        HWAPI_LOGE(PLUGIN_ID, "git revert 失败: %s", r->stdout_buf ? r->stdout_buf : "");
    git_result_free(r);
    return ret;
}