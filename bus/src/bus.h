...
typedef struct hw_plugin_discovery {
    ...
    char *provides[32];  int provides_count;
    char *requires[32];  int requires_count;
    char *conflicts[16]; int conflicts_count;
    char *files[64];     int files_count;
    size_t size;
 ...
 * BUS 主接口
 * ============================================================ */
extern int  hw_bus_init(hw_bus_t *bus, const char *state_dir,
                        const char *scan_path, int level);
...
extern int  hw_bus_resolve(hw_bus_t *bus, const char *protocol,
