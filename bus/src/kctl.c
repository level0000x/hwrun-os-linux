...
#define KCTL_IOC(dir, type, nr, size)                                                              \
    (((uint32_t)(dir) << KCTL_DIR_SHIFT) | ((uint32_t)(type) << KCTL_TYPE_SHIFT) |                 \
     ((uint32_t)(nr) << KCTL_NR_SHIFT) | ((uint32_t)(size) << KCTL_SIZE_SHIFT))
...
int hwrun_kctl_protocol_register(hwrun_kctl_t *c, const char *protocol, const char *version,
                                 const char *provider) {
...
int hwrun_kctl_protocol_unregister(hwrun_kctl_t *c, const char *protocol, const char *provider) {
...
int hwrun_kctl_protocol_resolve(hwrun_kctl_t *c, const char *protocol, hwrun_kctl_desc_t *out) {
