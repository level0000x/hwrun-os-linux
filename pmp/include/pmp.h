...
typedef struct hw_pmp_ops {
    ...
    /* ---------- 调度 ---------- */
    int   (*sched_set)(int32_t pid, pmp_sched_policy_t policy,
                       int32_t priority, int32_t nice);
    int   (*sched_get)(int32_t pid, pmp_sched_policy_t *policy,
    ...
    uint64_t (*task_submit)(const char *name, const char *path,
