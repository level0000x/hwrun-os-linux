...
static int pmp_plugin_init(hw_plugin_t *self) {
    ...
    g_ctx.initialized = 1;
    HWAPI_LOGI("pmp", "init: task_pool=%d", HWAPI_PARAM_GET_INT("pmp.task_pool", 32));
...
static int pmp_plugin_configure(hw_plugin_t *self, const char *key, const char *value) {
    (void)self;
    (void)key;
    (void)value;
    return HWRUN_OK;
...
static void *pmp_get_interface(const char *protocol) {
    ...
    if (strcmp(protocol, HWPROTO_PMP) == 0) return &g_pmp_ops;
    return NULL;
}
...
HWRUN_PLUGIN_DEFINE("pmp", "Process Management Protocol", "1.0.0", HWPLUGIN_TYPE_KERNEL,
                    "HWRun OS 进程管理协议：进程创建/控制/调度/信号/任务管理", &pmp_ops, g_provides,
                    g_requires)