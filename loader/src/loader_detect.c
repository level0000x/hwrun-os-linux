 ...
 * ============================================================ */
int loader_detect_impl(const char *path, loader_format_t *fmt, char *format_name, size_t name_size,
    ...
    if (rlen >= 2 && hdr[0] == '#' && hdr[1] == '!') {
        ...
        if (interpreter && interp_size > 0) {
            ...
            const char *end = s;
            while ((size_t)(end - (const char *)hdr) < (size_t)rlen && *end && *end != '\n' &&
                   *end != ' ' && *end != '\t')
                end++;
    ...
    for (i = 0; i < (int)MAGIC_COUNT; i++) {
        ...
        if ((long)e->magic_len <= rlen && memcmp(hdr, e->magic, e->magic_len) == 0) {
            if (fmt) *fmt = e->format;
            if (format_name && name_size) snprintf(format_name, name_size, "%s", e->name);
            if (interpreter && interp_size) interpreter[0] = '\0';
            return 0;
        }
    }
