...
static int parse_ph32(const uint8_t *m, long len, int be, const ehdr_view_t *eh,
                      loader_elf_info_t *out) {
    int i;
...
static int parse_ph64(const uint8_t *m, long len, int be, const ehdr_view_t *eh,
                      loader_elf_info_t *out) {
    int i;
    uint16_t phnum = eh->e_phnum;
    if (!phnum || eh->e_phoff == 0) return 0;
...
/* ---- 读取并解析节名字符串表 ---- */
static void parse_sections(const uint8_t *m, long len, int be, const ehdr_view_t *eh,
                           loader_elf_info_t *out) {
    ...
    out->section_names[0] = '\0';
    {
        ...
        for (i = 0; i < shnum && i < 40; i++) {
            ...
            if (stroff && strsz && nameoff < strsz) {
                ...
                while ((size_t)(nstr + nlen) < (size_t)m + (size_t)len && nstr[nlen] != '\0' &&
                       nlen < sizeof(tmp) - 1)
                    nlen++;
            } else {
                nstr = NULL;
            ...
            if (nlen > 0) {
                ...
                if (pos == 0) {
                    pos += (size_t)snprintf(out->section_names + pos,
                                            sizeof(out->section_names) - pos, "%s", tmp);
...
int loader_elf_impl(const char *path, loader_elf_info_t *out) {
    ...
    /* 节名概要 */
    if (eh.e_shnum && eh.e_shoff) parse_sections(m, len, be, &eh, out);
